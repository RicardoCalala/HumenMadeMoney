...
Each reviewer must be an adult, able to read the packet in an available accessible format, familiar enough with Human Made Money to distinguish an agreement, evidence, an advisory assessment, human review, and settlement authority, and able to attest that they completed the work independently. The approved and frozen `human-review-orientation-v1` in [`docs/human-review-orientation-v1.md`](human-review-orientation-v1.md) is the exact product and AI-policy orientation. `OPR-HMM00001` presents its marked orientation text verbatim and then asks its neutral restatement question exactly as written for every candidate. Anyone who cannot correctly restate the existing advisory-only boundary after orientation is excluded.
...
Before eligibility is finalized, the operator follows `human-review-orientation-v1`: preserve any earlier restatement response and outcome, present the frozen text verbatim with approved accessibility support or extra time as needed, do not coach or answer the question, ask the exact neutral question, and preserve the first post-orientation response separately. A failed, incomplete, or not-demonstrated attempt is never erased or overwritten. The orientation remains separate from cases, orders, answer keys, expected answers, scoring materials, and other reviewer responses.
...
This orientation clarification does not revise this frozen protocol or `human-review-rubric-v1`, `hmm-comprehension-study-v1`, `human-review-result-v1`, dataset `2.0.0`, cases, presentation orders, sample size, thresholds, partition coverage, eligibility criteria, authority semantics, or the `NOT_QUALIFIED` state.
