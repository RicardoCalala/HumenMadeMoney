...
import { errorResponse, etag, json, parseIfMatch, readJson, requestContext, requireAuthenticatedCsrf } from "../../../../../server/agreements/transport/http.ts";

export const dynamic = "force-dynamic";
type Context = { params: Promise<{ agreementId: string }> };
export async function GET(request: Request, route: Context) { const context = await requestContext(request); try { const { agreementId } = await route.params; const resource = await agreementService.get(context, agreementId); return json(resource, 200, { ETag: etag(resource.currentVersionId) }); } catch (error) { return errorResponse(error, context); } }
