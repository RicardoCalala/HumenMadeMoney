...
const create = () => { const repository = selectPersistenceAdapter() === "prisma" ? new PrismaWorkflowRepository(getPrismaClient()) : new InMemoryWorkflowRepository(); return { repository, service: new EvidenceAssessmentService(repository, () => new Date(), (kind) => `${kind}-${crypto.randomUUID()}`) }; };
