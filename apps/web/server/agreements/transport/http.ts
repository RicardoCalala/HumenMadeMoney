...
const safeContextId = (value: string | null, fallback: string) => value && SAFE_CONTEXT_ID.test(value) ? value : fallback;
export async function requestContext(request: Request): Promise<RequestContext> { const resolved = await authenticationService.resolve(cookieValue(request, SESSION_COOKIE)); const requestId = safeContextId(request.headers.get("x-request-id"), resolved.context.requestId); return { ...resolved.context, requestId, correlationId: safeContextId(request.headers.get("x-correlation-id"), requestId) }; }
