...
export function selectAdvisoryAssessmentProvider(env: NodeJS.ProcessEnv, dependencies: { openAiTransport?: OpenAiTransport } = {}): AdvisoryAssessmentProvider {
  ...
  if (config.provider !== "openai" || !dependencies.openAiTransport) throw new Error("AI_PROVIDER_REGISTRATION_FAILED");
  return new OpenAiAssessmentAdapter(dependencies.openAiTransport, config);
}
