...
import { errorResponse, etag, json, readJson, requestContext, requireAuthenticatedCsrf } from "../../../../server/agreements/transport/http.ts";
...
export async function GET(request: Request) { const context = await requestContext(request); try { return json(await agreementService.list(context, parseListQuery(new URL(request.url)))); } catch (error) { return errorResponse(error, context); } }
