...
export class InvocationGuard {
  ...
  enter(actorId: string, tool: string, now: number) { const window = Math.floor(now / 60); const key = `${actorId}:${window}`; const current = this.calls.get(key) ?? { window, count: 0 }; if (current.count >= 60) throw new EvidenceError("RESOURCE_LIMIT", "The local MCP request limit was reached.", 429); current.count += 1; this.calls.set(key, current); const lease = `${actorId}:${tool}`; if (this.leases.has(lease)) throw new EvidenceError("CONCURRENCY_CONFLICT", "A matching local MCP request is already running.", 409); this.leases.add(lease); return () => this.leases.delete(lease); }
}
